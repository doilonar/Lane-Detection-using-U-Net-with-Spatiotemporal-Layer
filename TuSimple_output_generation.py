...
def fillCurves(img_shape, pts_left, pts_right):
    ...
    cv2.polylines(img, np.int32([pts_right]), isClosed=False, color=(255, 255, 255), thickness=10)
    cv2.polylines(img, np.int32([pts_left]), isClosed=False, color=(255, 255, 255), thickness=10)
    return img
...
def addTurnInfo(img, radius):
    if radius >= 10000:
        img = cv2.putText(img, 'Inainte', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2, cv2.LINE_AA)
    if radius >= 0 and radius < 10000:
        img = cv2.putText(img, 'Dreapta', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2, cv2.LINE_AA)
    if radius <= 0:
        img = cv2.putText(img, 'Stanga', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2, cv2.LINE_AA)
...
def apply_filter(frame):
    ...
    source_points = np.float32([[500,300 ], [750,300], [200,690 ], [1260,630 ]])
    sp = source_points
    destination_points = np.float32([[0,0], [sp[2][1]-sp[1][1],0], [0, sp[3][0]-sp[2][0]], [sp[2][1]-sp[1][1], sp[3][0]-sp[2][0]]])
    warped_img_size = (int(sp[2][1]-sp[1][1]), int(sp[3][0]-sp[2][0]))
    warped_img_shape = ( int(sp[3][0]-sp[2][0]), int(sp[2][1]-sp[1][1]))
    ...
    img_filter = np.zeros_like(frame)
    window1 = cv2.addWeighted(img_filter, 1, unwarped_fill_curves, 1, 0)
...
in_folder = r'0530'
elem_dirs = os.listdir(in_folder)
err = 0
for director in elem_dirs:
    in_folder = r'0530\\' + str(director)
    out_folder = r'lane_detect\\' + str(director)
    out_lane = r'lane\\' + str(director)
    if  os.path.exists(out_folder):
        #os.makedirs(out_folder)
        #os.makedirs(out_lane)
        nr = 1
        ...
        for filename in os.listdir(in_folder):
            if filename.endswith(('.jpg', '.jpeg', '.png')):
                    ...
                    img_filter = apply_filter(img)
                    plt.axis('off')
                    plt.imshow(img_filter,cmap='gray')
                    plt.show()
                    #plt.savefig(os.path.join(out_folder,str(nr) + '.jpg'),bbox_inches='tight', pad_inches=0.0)
                    plt.close('all')
                    plt.axis('off')
                    plt.imshow(img)
                    #plt.savefig(os.path.join(out_lane,str(nr)+'.jpg'),bbox_inches='tight', pad_inches=0.0)
                    print(out_folder)
                    nr += 1
